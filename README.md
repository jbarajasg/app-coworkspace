...
Esa separación compra tres cosas concretas, demostrables en este repo:

1. **Testabilidad total del negocio.** Las reglas son funciones puras: se testean sin `TestBed`, sin mocks de Angular y con el reloj inyectado — por eso hay tests deterministas de bordes como "exactamente 24 h" o "reservas consecutivas".
2. **Infraestructura intercambiable.** Las pantallas y stores dependen de puertos, no de implementaciones. Pasar del mock a un backend real es escribir un adaptador HTTP y cambiar dos providers; los tests de integración ya ejercitan ese swap usando fakes por los mismos puertos.
3. **Cambios locales.** Cuando se ajustó la semántica de RN-01 (ver decisiones), el cambio fue una función de una línea y la suite señaló el resto. Con la lógica repartida por componentes, habría sido una búsqueda manual por toda la UI.

---

## Decisiones de negocio documentadas

**Rangos consecutivos son conflicto (intervalos cerrados).** El enunciado dice literalmente que ante una reserva de 10:00–12:00 _"no se puede reservar de 11:00 a 13:00 **ni de 12:00 a 14:00**".
---
